--rbxsig%oLQ/VFgHL+wLQ+EUuLpepfdaw9RE0lIrvR6e4BJFNpYle5fG/qi8VIy2qk0+uMjjwRobHoCJEd4dR5G3CIhWVScR/rNKCRpIYysoz5OaIDWBK8vaHdRsupWSctETWENl6sr+HeTY9PBKsBPwLGizggdh/++AvkqpIYCoaepRPx4=%
-- Start Game Script Arguments
local placeId, port, gameId, sleeptime, access, deprecated, timeout, machineAddress, gsmInterval, baseUrl, maxPlayers, maxGameInstances, injectScriptAssetID, apiKey, libraryRegistrationScriptAssetID, pingTimesReportInterval, gameCode, universeId, preferredPlayerCapacity, matchmakingContextId, placeVisitAccessKey, assetGameSubdomain, protocol = ...
...
game:GetService("Players").PlayerAdded:connect(function(player)
	...
	if assetGameUrl and access and placeId and player and player.userId then
		...
		game:HttpPost(assetGameUrl .. "/Game/PlaceVisit.ashx?UserID=" .. player.userId .. "&AssociatedPlaceID=" .. placeId .. "&placeVisitAccessKey=" .. placeVisitAccessKey, "")
	end
end)
...
game:GetService("Players").PlayerRemoving:connect(function(player)
	...
	if assetGameUrl and access and placeId and player and player.userId then
		game:HttpGet(assetGameUrl .. "/Game/ClientPresence.ashx?action=disconnect&PlaceID=" .. placeId .. "&UserID=" .. player.userId)
	end
end)
